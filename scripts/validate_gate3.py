...
def main() -> int:
    ...
    for path, record in records:
        ...
        row = {
            ...
            "has_context": bool(record.get("corpus_references")),
    ...
    metrics = {
        ...
        "with_primary_attestation": sum(r["attestations_count"] > 0 for r in index_rows),
        "with_two_historical_sources": sum(r["sources_count"] >= 2 for r in index_rows),
        "with_context": sum(r["has_context"] for r in index_rows),
        ...
        "with_conflicting_claims": sum(r["has_conflict"] for r in index_rows),
